...
export default function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  </nav><button className='mt-6 text-sm underline' onClick={() => { clearTokens(); router.push('/login'); }}>Sign out</button></aside><main className='flex-1'><header className='border-b bg-white px-4 py-3'><div className='font-semibold'>{role === 'COMMUNITY_ADMIN' ? 'Community Management' : 'Community Flag Scheduler Administration'}</div><div className='text-sm text-slate-500'>{APP_SUBTITLE}</div></header><section className='p-4'>{user?.missing_score_reminder ? <div className='mb-4 rounded border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950'><div className='font-semibold'>{user.missing_score_reminder.message}</div>{user.missing_score_reminder.recent_games?.length ? <ul className='mt-2 list-disc pl-5'>{user.missing_score_reminder.recent_games.slice(0, 3).map((game) => <li key={game.game_id}>{game.away_team_name} at {game.home_team_name}</li>)}</ul> : null}<Link className='mt-3 inline-block rounded bg-amber-700 px-3 py-2 text-white' href={user.missing_score_reminder.link}>{user.missing_score_reminder.action_label || 'Go to Score Management'}</Link></div> : null}{children}</section></main></div>;
}
