...
export default function FormField({ label, type = 'text', value, onChange, options }: Props) {
  ...
  if (type === 'textarea') return <label className='text-sm'><span className='mb-1 block font-medium'>{label}</span><textarea className='w-full rounded border p-2' value={value ?? ''} onChange={(e) => onChange(e.target.value)} /></label>;
