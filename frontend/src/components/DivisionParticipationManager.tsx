...
export default function DivisionParticipationManager() {
  ...
  useEffect(() => { (async () => {
    ...
    try {
      setDivisionsError('');
      const divResp = await apiFetch('/divisions?page_size=500', { cache: 'no-store' }, getToken());
      setDivisions((divResp.items || []).filter((d: any) => d.is_active));
    } catch (e: any) {
      const message = e instanceof ApiError ? 'Unable to load divisions. Check backend logs.' : (e?.message || 'Unable to load divisions. Check backend logs.');
      setDivisions([]);
      setDivisionsError(message);
    }
  })(); }, []);
  ...
  useEffect(() => { if (!orgId) return; (async () => {
    try {
      ...
      setMsg('');
      const resp = await apiFetch(`/organization-division-participation?organization_id=${orgId}`, { cache: 'no-store' }, getToken());
      const map: Record<string, Participation> = {};
      for (const d of divisions) map[d.id] = { division_id: d.id, team_count: 0 };
      for (const p of resp) map[p.division_id] = { division_id: p.division_id, team_count: p.team_count ?? 0 };
      setRows(map);
    } catch (e: any) {
      const message = e instanceof ApiError ? 'Unable to load divisions. Check backend logs.' : (e?.message || 'Unable to load divisions. Check backend logs.');
      setMsg(message);
  ...
  return <div className='space-y-4'>
    ...
    {divisionsError && <p className='rounded border border-rose-200 bg-rose-50 p-2 text-sm text-rose-700'>{divisionsError}</p>}
    ...
    {orgId && (['COED', 'GIRLS'] as const).map((g) => <div key={g} className='rounded border p-3'>
      ...
      <table className='w-full text-sm'><thead><tr className='text-left'><th>Division</th><th>Number of Teams</th><th>Participation</th></tr></thead><tbody>
        {groups[g].map((d) => {
          const teamCount = rows[d.id]?.team_count ?? 0;
          ...
          return <tr key={d.id} className='border-t'><td className='py-2'>{d.name}</td><td className='py-2'>
            <input type='number' min={0} step={1} className='w-24 rounded border p-1' value={teamCount}
              onChange={(e) => {
                ...
                setRows({ ...rows, [d.id]: { division_id: d.id, team_count: Number.isNaN(parsed) ? 0 : parsed } });
              }} />
          </td><td className='py-2'>{participating ? 'Participating' : 'Not participating'}</td></tr>;
        })}
      </tbody></table>
    </div>)}
    <button disabled={!orgId || participationLoading || !!divisionsError} className='rounded bg-emerald-700 px-4 py-2 text-white disabled:opacity-50' onClick={save}>Save Participation</button>
