    ...
    <div className='space-y-4'>
      ...
      <div className='overflow-auto rounded border bg-white'>
        <table className='min-w-full text-sm'>
          ...
          <tbody>{items.map((week: any) => <tr key={week.id} className='border-b align-top'><td className='p-2'>{seasonName(week.season_id)}</td><td className='p-2'>Week {week.week_number}</td><td className='p-2'>{weekOptionLabel(week)}</td><td className='p-2'>{formatDate(week.start_date)}</td><td className='p-2'>{formatDate(week.end_date)}</td><td className='p-2'>{formatDate(week.primary_game_date)}</td><td className='p-2'><span className={`rounded-full border px-2 py-1 text-xs font-semibold ${statusClass[week.status] || statusClass.draft}`}>{titleCase(week.status || 'draft')}</span></td><td className='p-2'>{week.hosting_availability_count ?? 0} host sites</td><td className='p-2'>{week.generated_slots_count ?? 0} slots</td><td className='p-2'>{week.scheduled_games_count ?? 0} games</td><td className='p-2'><div className='flex min-w-48 flex-col gap-1'><button className='text-left text-blue-700 underline' onClick={() => edit(week)}>Edit Week</button><Link className='text-blue-700 underline' href={`/admin/hosting-availability?week_id=${week.id}&start_date=${week.start_date}&end_date=${week.end_date}`}>View Hosting Availability</Link><Link className='text-blue-700 underline' href={`/admin/generated-slots?week_id=${week.id}&start_date=${week.start_date}&end_date=${week.end_date}`}>View Generated Slots</Link><Link className='text-blue-700 underline' href={`/admin/schedule-readiness?week_id=${week.id}&start_date=${week.start_date}&end_date=${week.end_date}`}>View Schedule Readiness</Link><button className='text-left text-blue-700 underline' onClick={() => lockWeek(week)}>Lock Week</button><button className='text-left text-blue-700 underline' onClick={() => duplicate(week)}>Duplicate Week Setup</button><Link className='text-blue-700 underline' href={`/admin/manual-schedule-builder?season_id=${week.season_id}&week_id=${week.id}`}>Manual Schedule Builder</Link><Link className='text-blue-700 underline' href={`/admin/schedule-management?week_id=${week.id}`}>Schedule Management</Link><Link className='text-blue-700 underline' href={`/schedule?week_id=${week.id}`}>Published Schedule</Link></div></td></tr>)}</tbody>
        </table>
      </div>
    </div>
  );
