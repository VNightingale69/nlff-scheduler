...
export default function CommunityScoreEntryPage() {
  ...
  return <div className='space-y-4'><div><h1 className='text-2xl font-bold'>Score Entry</h1><p className='text-sm text-slate-600'>Submit scores inline for scheduled games involving your community as either the home or away team.</p><p className='text-xs text-slate-500'>Enter a number, leave blank for 0, or enter F for a forfeit.</p></div>{message && <div className='rounded border bg-green-50 p-3 text-sm'>{message}</div>}<div className='flex gap-2'><select className='rounded border p-2' value={status} onChange={(e) => setStatus(e.target.value)}><option value=''>All statuses</option>{statuses.map((s) => <option key={s}>{s}</option>)}</select><button className='rounded bg-slate-800 px-3 py-2 text-white' onClick={load}>Apply</button></div><div className='overflow-x-auto rounded border bg-white'><table className='min-w-full text-sm'><thead className='bg-slate-100 text-left'><tr>{['Date','Time','Division','Host Location','Field','Home Team','Away Team','Home Score','Away Score','Submitted By','Submitted','Score Status','Published Status','Updated','Actions'].map((h) => <th key={h} className='p-2'>{h}</th>)}</tr></thead><tbody>{items.map((g) => { const locked = ['APPROVED','PUBLISHED','CONFLICT','CORRECTION_PENDING'].includes(g.score_status) || g.is_published; const d = draftFor(g); return <tr key={g.game_id} className='border-t'><td className='p-2'>{formatDisplayDate(g.game_date)}</td><td className='p-2'>{formatDisplayTime(g.kickoff_time)}</td><td className='p-2'>{g.division_group} {g.division_name}</td><td className='p-2'>{g.host_location_name}</td><td className='p-2'>{g.field_name}</td><td className='p-2'>{g.home_team_name}</td><td className='p-2'>{g.away_team_name}</td><td className='p-2'><input className='w-20 rounded border p-1' type='text' inputMode='numeric' value={d.home_score} disabled={locked} onChange={(e) => setDraft(g, { home_score: e.target.value })} /></td><td className='p-2'><input className='w-20 rounded border p-1' type='text' inputMode='numeric' value={d.away_score} disabled={locked} onChange={(e) => setDraft(g, { away_score: e.target.value })} /></td><td className='p-2'>{g.submitted_by?.full_name || ''}</td><td className='p-2'><ScoreTimestamp value={g.submitted_at} /></td><td className='p-2'>{g.score_status}</td><td className='p-2'>{g.is_published ? 'Published' : 'Unpublished'}</td><td className='p-2'><ScoreTimestamp value={g.last_updated_at || g.submitted_at} /></td><td className='space-x-2 whitespace-nowrap p-2'>{locked ? <button className='rounded border px-2 py-1' onClick={() => flag(g)}>Flag Score Issue</button> : <button className='rounded bg-slate-800 px-2 py-1 text-white' onClick={() => submit(g)}>{g.score_status === 'SUBMITTED' || g.score_status === 'FLAGGED' ? 'Save Score' : 'Submit Score'}</button>}<span className='text-slate-500'>View Status</span></td></tr>; })}</tbody></table></div></div>;
}
