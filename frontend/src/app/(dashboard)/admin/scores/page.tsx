...
export default function ScoreManagementPage() {
  ...
  const history = async (g: ScoreGame) => { const data = await apiFetch(`/scores/${g.game_id}/history`, {}, token); alert((data.items || []).map((h: any) => `${formatDateTime(h.created_at)}: ${h.action} ${h.previous_away_score ?? ''}-${h.previous_home_score ?? ''} → ${h.new_away_score ?? ''}-${h.new_home_score ?? ''} (${h.previous_status} → ${h.new_status}) by ${h.actor?.full_name || h.actor_user_id || 'system'}${h.reason ? ` — ${h.reason}` : ''}`).join('\n') || 'No score history yet.'); };
  ...
  return <div className='space-y-4'>
    ...
    <div className='overflow-x-auto rounded border bg-white'><table className='min-w-full text-sm'><thead className='bg-slate-100 text-left'><tr>{['Date','Time','Division','Host Location','Field','Home Team','Away Team','Home Score','Away Score','Submitted By','Submitted','Score Status','Published Status','Updated','Actions'].map((h) => <th key={h} className='p-2'>{h}</th>)}</tr></thead><tbody>{items.map((g) => { const d = draftFor(g); return <tr key={g.game_id} className='border-t align-top'><td className='p-2'>{formatDisplayDate(g.game_date)}</td><td className='p-2'>{formatDisplayTime(g.kickoff_time)}</td><td className='p-2'>{g.division_group} {g.division_name}</td><td className='p-2'>{g.host_location_name}</td><td className='p-2'>{g.field_name}</td><td className='p-2'>{g.home_team_name}</td><td className='p-2'>{g.away_team_name}</td><td className='p-2'><input className='w-20 rounded border p-1' type='text' inputMode='numeric' value={d.home_score} onChange={(e) => setDraft(g, { home_score: e.target.value })} /></td><td className='p-2'><input className='w-20 rounded border p-1' type='text' inputMode='numeric' value={d.away_score} onChange={(e) => setDraft(g, { away_score: e.target.value })} /></td><td className='p-2'>{g.submitted_by?.full_name || ''}</td><td className='p-2'><ScoreTimestamp value={g.submitted_at} /></td><td className='p-2'>{g.score_status}</td><td className='p-2'>{g.is_published ? 'Published' : 'Unpublished'}</td><td className='p-2'><ScoreTimestamp value={g.last_updated_at || g.approved_at || g.submitted_at} /></td><td className='flex flex-wrap gap-1 p-2'><button className='rounded border px-2 py-1' onClick={() => save(g)}>Save Correction</button><button className='rounded border px-2 py-1' onClick={() => approve(g)}>Approve</button><button className='rounded border px-2 py-1' onClick={() => publish(g)}>Publish</button><button className='rounded border px-2 py-1' onClick={() => approvePublish(g)}>Approve & Publish</button><button className='rounded border px-2 py-1' onClick={() => unpublish(g)}>Unpublish</button><button className='rounded border px-2 py-1' onClick={() => resolve(g)}>Resolve Conflict</button><button className='rounded border px-2 py-1' onClick={() => flag(g)}>Flag</button><button className='rounded border px-2 py-1' onClick={() => clear(g)}>Clear Score</button><button className='rounded border px-2 py-1' onClick={() => history(g)}>View History</button></td></tr>; })}</tbody></table></div>
  </div>;
}
