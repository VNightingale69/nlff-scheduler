...
type Game = { date: string; kickoff: string; division: string; home_team: string; away_team: string; host_organization?: string; host_location?: string; field: string; field_type?: string; physical_area?: string; publication_status: string; home_team_logo_url?: string; home_team_logo_alt_text?: string; away_team_logo_url?: string; away_team_logo_alt_text?: string };
...
export default function ScheduleReviewPage() {
  ...
  return <div className='mx-auto w-full max-w-[1800px] space-y-5 print:max-w-none'>
    ...
    {!loading && !error && payload.weeks.map((week) => <section key={week.week_id} className='space-y-3 rounded border bg-white p-4 break-inside-avoid'>
      ...
      {!week.games.length ? <p className='text-sm text-slate-500'>No games match the selected filters.</p> : view === 'hosting' ? <HostingView mode='unpublished' games={week.games.map((game, index): HostingViewGame => ({ id: `${week.week_id}-${index}`, date: game.date, time: game.kickoff, hostLocation: game.host_location || 'Host Location Unassigned', physicalArea: game.physical_area || game.field, fieldLane: game.field, fieldType: game.field_type, division: game.division, homeTeam: game.home_team, awayTeam: game.away_team, homeLogoUrl: game.home_team_logo_url, homeLogoAlt: game.home_team_logo_alt_text, awayLogoUrl: game.away_team_logo_url, awayLogoAlt: game.away_team_logo_alt_text }))} /> : <GameTable games={[...week.games].sort((a, b) => groupValue(a, view).localeCompare(groupValue(b, view)) || a.kickoff.localeCompare(b.kickoff))} groupBy={view} />}
    </section>)}
  </div>;
}
