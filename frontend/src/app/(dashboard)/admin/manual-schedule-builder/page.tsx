...
const INTERNAL_TURF_LAYOUT_PATTERN = /\b(?:ONE_LARGE_ONE_SMALL|THREE_SMALL|TWO_MEDIUM|ONE_SMALL_ONE_LARGE|TWO_SMALL_ONE_MEDIUM|ONE_LARGE|ONE_MEDIUM_TWO_SMALL|ONE_LARGE_ONE_MEDIUM|TWO_LARGE)\b/gi;
...
function isMissingFieldAssignment(value: any): boolean {
  // The canonical selection is the effective assignment, including while an
  // editable row is unsaved. Legacy warning flags must not make a selected
  // field simultaneously appear to be missing.
  if (value?.field_id || value?.field_instance_id || value?.generated_field_instance_id) return false;
  return true;
}

function explicitFieldSlotLabel(value: any): string {
  if (isMissingFieldAssignment(value)) return 'Field Not Assigned';
...
export default function ManualScheduleBuilderPage() {
  ...
  const scheduledGamesFilterOptions = useMemo(() => {
    ...
    const fieldSourceGames = scheduledGamesFilters.hostLocation
      ...
      : games;
    const fields = uniqueByValue(fieldSourceGames.map((game: any) => ({ value: explicitFieldSlotLabel(game), label: isMissingFieldAssignment(game) ? 'Missing Field / Field Not Assigned' : (explicitFieldSlotLabel(game) || '-') })))
      .sort((a, b) => naturalCollator.compare(a.label, b.label));
  ...
  const performDelete = async (game: any) => {
    setError('');
    setSuccess('');
    try {
      await apiFetch(`/games/${game.id}`, { method: 'DELETE' }, token);
      setDeleteGame(null);
      setPendingGameEdits((current) => { const next = { ...current }; delete next[game.id]; return next; });
      await load();
      await loadRecommendations();
      await loadGeneratedSlots();
      setManualScheduleBannerFromValidation('Game deleted.', await loadFinalScheduleValidation());
    } catch (e: unknown) { setError(extractError(e)); }
  };
  const isPublishedGame = (game: any) => String((options.weeks || []).find((week: any) => String(week.id) === String(game.week_id))?.publication_status || '').toUpperCase() === 'PUBLISHED';
  ...
  const getFieldOptionsForPendingEdit = (pendingEdit: any) => {
    ...
    const divisionForRow = (options.divisions || []).find((item: any) => String(item.id) === String(pendingEdit?.division_id || ''));
    const requiredFieldType = String(divisionForRow?.required_field_type || '').toUpperCase();
    const hostFields = (options.fields || [])
      .filter((field: any) => String(field.host_location_id || '') === selectedHostId && field.is_active);
    const values = hostFields
      .filter((field: any) => !requiredFieldType || String(field.field_type || '').toUpperCase() === requiredFieldType)
      .map((field: any) => ({ ...field, explicit_field_slot_label: field.display_name || field.name }));
    // Keep a valid existing assignment selectable while editing historical
    // games, even when the division's current default type has changed.
    if (pendingEdit?.field_id && !values.some((field: any) => String(field.id) === String(pendingEdit.field_id))) {
      const assigned = hostFields.find((field: any) => String(field.id) === String(pendingEdit.field_id));
      values.push(assigned
        ? { ...assigned, explicit_field_slot_label: assigned.display_name || assigned.name }
        : { id: pendingEdit.field_id, field_id: pendingEdit.field_id, host_location_id: pendingEdit.host_location_id, explicit_field_slot_label: cleanVisibleFieldLabel(pendingEdit.field_name || pendingEdit.field_instance_name || 'Current field'), field_type: pendingEdit.field_type });
    }
  ...
  return (
    <div className='space-y-4'>
          ...
          <table className='min-w-full text-sm'>
            ...
            <tbody>
              {filteredGames.map((g: any) => {
                ...
                const selectedHost = options.host_locations.find((host: any) => String(host.id) === String(pendingEdit.host_location_id || ''));
                const activeHostFields = (options.fields || []).filter((field: any) => field.is_active && String(field.host_location_id) === String(pendingEdit.host_location_id || ''));
                const fieldEmptyMessage = !pendingEdit.host_location_id
                  ? 'Select a host location to load fields.'
                  : activeHostFields.length === 0
                    ? `No active fields configured for ${selectedHost?.name || 'this host location'}.`
                    : rowFieldOptions.length === 0
                      ? `No active ${divisionDefaultFieldType || 'compatible'} fields are available at ${selectedHost?.name || 'this host location'}.`
                      : '';
                ...
                return <tr key={g.id} className={`border-t align-top ${isDirtyRow ? 'bg-amber-50/40 outline outline-1 outline-amber-200' : ''}`}>
                  ...
                  <td className={`p-2${dirtyCellClass(pendingEdit, 'field_id')}`}>{editable ? <div className='space-y-1'><select aria-label={`Field for ${g.home_team_name || 'game'}`} className={compactSelectClass} disabled={!pendingEdit.host_location_id || rowFieldOptions.length === 0} value={pendingEdit.field_id || ''} onChange={(e) => { const selected = rowFieldOptions.find((field: any) => String(field.field_id || field.id) === e.target.value); const instance = fieldInstanceFor(e.target.value, pendingEdit.game_date); updatePendingEditForGame(g, { field_id: e.target.value, field_instance_id: instance?.field_instance_id || '', host_location_id: selected?.host_location_id || pendingEdit.host_location_id }); }}><option value=''>{fieldEmptyMessage || 'Field'}</option>{rowFieldOptions.map((field: any) => <option key={field.field_id || field.id} value={field.field_id || field.id}>{field.explicit_field_slot_label}</option>)}</select>{fieldEmptyMessage ? <div className='text-[11px] text-amber-700'>{fieldEmptyMessage}</div> : null}<div className={`text-[11px] ${fieldSizeMismatch ? 'text-amber-700' : 'text-slate-500'}`}>{fieldSizeLabel}</div>{isMissingFieldAssignment(pendingEdit) ? <span className='inline-flex rounded bg-amber-100 px-2 py-0.5 text-[11px] font-semibold text-amber-800'>Missing Field</span> : isDirtyCell(pendingEdit, 'field_id') ? <span className='inline-flex rounded bg-blue-100 px-2 py-0.5 text-[11px] font-semibold text-blue-800'>Unsaved</span> : null}</div> : <div className='space-y-1'><div>{explicitFieldSlotLabel(g) || '-'}</div>{isMissingFieldAssignment(g) ? <span className='inline-flex rounded bg-amber-100 px-2 py-0.5 text-[11px] font-semibold text-amber-800'>Missing Field</span> : null}</div>}</td>
                  ...
                  {canManageGeneratedGames ? <td className='p-2'><div className='flex flex-wrap gap-2'>
                    {isBulkEditMode ? <>
                      ...
                      <button className='rounded border border-red-300 px-2 py-1 text-xs text-red-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:text-slate-400' disabled={isPublishedGame(g)} title={isPublishedGame(g) ? 'Published games cannot be deleted from the Manual Schedule Builder.' : 'Delete this unpublished scheduled game'} onClick={() => {
                        setAutoFillSkipped([]);
                        setAutoFillPreview([]);
                        setDeleteGame(g);
                      }}>Delete Game</button>
      ...
      {deleteGame ? <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4'>
        <div className='w-full max-w-lg rounded-lg bg-white p-5 shadow-xl' role='dialog' aria-modal='true' aria-labelledby='delete-game-title'>
          <h3 id='delete-game-title' className='text-lg font-semibold text-red-800'>Delete Scheduled Game?</h3>
          <div className='mt-3 space-y-1 rounded border bg-slate-50 p-3 text-sm text-slate-800'>
            <p className='font-semibold'>{formatDisplayDateTime(deleteGame.game_date, deleteGame.kickoff_time)}</p>
            <p>{deleteGame.division_name || 'Unknown Division'}</p>
            <p>{deleteGame.home_team_name || 'Unknown Team'} vs {deleteGame.away_team_name || 'Unknown Team'}</p>
            <p>{deleteGame.host_location_name || 'No host location'}</p>
            <p>Field: {explicitFieldSlotLabel(deleteGame) || 'Not assigned'}</p>
          </div>
          {doubleheaderPartner(deleteGame) ? <p className='mt-3 rounded border border-amber-300 bg-amber-50 p-2 text-sm text-amber-900'><strong>Doubleheader warning:</strong> The other doubleheader game will not be deleted.</p> : null}
          <p className='mt-3 text-sm text-slate-700'>This game has not been published. Deleting it will remove it from the working schedule.</p>
          <div className='mt-4 flex flex-wrap justify-end gap-2'>
            <button className='rounded border px-3 py-2' onClick={() => setDeleteGame(null)}>Cancel</button>
            <button className='rounded bg-red-700 px-3 py-2 text-white' onClick={() => performDelete(deleteGame)}>Delete Game</button>
          </div>
        </div>
      </div> : null}
