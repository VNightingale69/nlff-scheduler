        ...
        {!weeklyDemand.length ? <p className='text-sm text-slate-500'>No weekly demand or generated capacity found yet.</p> : (
          <div className='space-y-3'>
            <div className='overflow-auto'><table className='min-w-full text-sm'>
              ...
              <tbody>{weeklyDemand.map((row) => <tr key={`summary-${row.host_date}`} className='border-b'><td className='p-2'>{row.week ?? '—'}</td><td className='p-2'>{row.host_date}</td><td className='p-2'>{row.required_by_size?.SMALL ?? row.small_games_required}</td><td className='p-2'>{row.status === 'DEFERRED' ? 'Deferred' : (row.eligible_by_size?.SMALL ?? 0)}</td><td className='p-2'>{row.required_by_size?.MEDIUM ?? row.medium_games_required}</td><td className='p-2'>{row.status === 'DEFERRED' ? 'Deferred' : (row.eligible_by_size?.MEDIUM ?? 0)}</td><td className='p-2'>{row.required_by_size?.LARGE ?? row.large_games_required}</td><td className='p-2'>{row.status === 'DEFERRED' ? 'Deferred' : (row.eligible_by_size?.LARGE ?? 0)}</td><td className={`p-2 font-semibold ${row.status === 'SHORT' ? 'text-rose-700' : 'text-emerald-700'}`}>{row.readiness_message || row.status}</td></tr>)}</tbody>
            </table></div>
