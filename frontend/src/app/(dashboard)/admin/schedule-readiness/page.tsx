        ...
        {!hostingRotation.length ? <p className='text-sm text-slate-500'>No host rotation data found yet.</p> : (
          <div className='space-y-2'>
            {hostingRotation.map((row) => <div key={row.week} className='rounded border bg-slate-50 p-3 text-sm'>
              ...
              <div>{row.capacity_diagnostic_label || 'Generated game-slot capacity'}: {row.community_capacity_by_field_size ? Object.entries(row.community_capacity_by_field_size).map(([community, capacity]: any) => `${community}: ${capacity.SMALL || 0} Small / ${capacity.MEDIUM || 0} Medium / ${capacity.LARGE || 0} Large`).join(' • ') : '—'}</div>
              <div>Host capacity: <span className={row.host_capacity_status === 'INVALID' ? 'font-semibold text-rose-700' : 'font-semibold text-emerald-700'}>{row.host_capacity_status || '—'}</span></div>
              <div>Saved schedule assignments: <span className={row.saved_schedule_blocking_issues?.length ? 'font-semibold text-rose-700' : 'font-semibold text-emerald-700'}>{row.saved_schedule_assignment_status || 'NOT SAVED'}</span></div>
        ...
        {!weeklyDemand.length ? <p className='text-sm text-slate-500'>No weekly demand or generated capacity found yet.</p> : (
          <div className='space-y-3'>
            <div className='overflow-auto'><table className='min-w-full text-sm'>
              <thead><tr className='border-b text-left'><th className='p-2'>Week</th><th className='p-2'>Date</th><th className='p-2'>Small games / peak</th><th className='p-2'>Eligible Small</th><th className='p-2'>Medium games / peak</th><th className='p-2'>Eligible Medium</th><th className='p-2'>Large games / peak</th><th className='p-2'>Eligible Large</th><th className='p-2'>Status</th></tr></thead>
              <tbody>{weeklyDemand.map((row) => <tr key={`summary-${row.host_date}`} className='border-b'><td className='p-2'>{row.week ?? '—'}</td><td className='p-2'>{row.host_date}</td><td className='p-2'>{row.required_by_size?.SMALL ?? row.small_games_required} games / {row.peak_simultaneous_by_size?.SMALL ?? 0} peak</td><td className='p-2'>{row.status === 'DEFERRED' ? 'Deferred' : (row.eligible_by_size?.SMALL ?? 0)}</td><td className='p-2'>{row.required_by_size?.MEDIUM ?? row.medium_games_required} games / {row.peak_simultaneous_by_size?.MEDIUM ?? 0} peak</td><td className='p-2'>{row.status === 'DEFERRED' ? 'Deferred' : (row.eligible_by_size?.MEDIUM ?? 0)}</td><td className='p-2'>{row.required_by_size?.LARGE ?? row.large_games_required} games / {row.peak_simultaneous_by_size?.LARGE ?? 0} peak</td><td className='p-2'>{row.status === 'DEFERRED' ? 'Deferred' : (row.eligible_by_size?.LARGE ?? 0)}</td><td className={`p-2 font-semibold ${row.status === 'SHORT' ? 'text-rose-700' : 'text-emerald-700'}`}>{row.readiness_message || row.status}</td></tr>)}</tbody>
            </table></div>
            {weeklyDemand.map((row) => <div key={row.host_date} className='rounded border bg-slate-50 p-3 text-sm'>
              ...
              <div>Games scheduled: {row.small_games_required} Small / {row.medium_games_required} Medium / {row.large_games_required} Large</div>
              <div>Peak simultaneous demand: {row.peak_simultaneous_by_size?.SMALL ?? 0} Small / {row.peak_simultaneous_by_size?.MEDIUM ?? 0} Medium / {row.peak_simultaneous_by_size?.LARGE ?? 0} Large</div>
              <div>{row.readiness_message || `Capacity: ${row.capacity_used ?? 0} used / ${row.capacity_available ?? 0} available`}</div>
