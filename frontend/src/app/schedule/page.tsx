...
export default function PublicSchedulePage() {
  ...
  const load = async () => { setLoading(true); const q = new URLSearchParams(Object.fromEntries(Object.entries(filters).filter(([, v]) => v))); const [gamesRes, optionsRes] = await Promise.all([fetch(`${API_URL}/public/games?${q.toString()}`), fetch(`${API_URL}/public/schedule-filters`)]); setGames(((await gamesRes.json()).items || [])); setOptions(await optionsRes.json()); setLoading(false); };
  useEffect(() => { load(); }, []);
  ...
  return <div className='mx-auto max-w-6xl space-y-4 p-4'><h1 className='text-2xl font-bold'>Northern Lakes Flag Football Public Schedule</h1>
    ...
    <div className='flex gap-2'><button className='rounded bg-slate-800 px-3 py-2 text-white' onClick={load}>Apply Filters</button><button className='rounded border px-3 py-2' onClick={()=>{setFilters({});}}>Reset</button></div>
