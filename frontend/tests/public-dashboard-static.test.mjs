...
for (const [label, href] of [['Schedule','/schedule'],['Standings','/standings'],['Scores','/scores'],['Locations','/locations'],['Rulebook','/rulebook']]) {
  assert.match(root, new RegExp(`href: '${href}', title: '${label}'`));
}
assert.match(root, /min-\[360px\]:grid-cols-2/);
assert.match(root, /lg:grid-cols-3/);
...
for (const page of ['app/schedule/page.tsx','app/standings/page.tsx','app/scores/page.tsx','app/locations/page.tsx','app/rulebook/page.tsx']) assert.match(read(page), /PublicLayout/);
