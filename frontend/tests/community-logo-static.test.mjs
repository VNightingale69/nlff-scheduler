...
const standingsTableSource = readFileSync(new URL('../src/components/StandingsTable.tsx', import.meta.url), 'utf8');
...
assert.match(organizationsSource, /logo_file_available/);
assert.match(organizationsSource, /logo_storage_error/);
assert.match(organizationsSource, /Logo metadata exists, but the image file could not be found\. Confirm persistent upload storage is configured, then replace the logo\./);
...
assert.match(standingsSource, /<StandingsTable rows=\{division\.standings\}/);
assert.match(standingsTableSource, /<CommunityLogo src=\{row\.community_logo_url\}/);
