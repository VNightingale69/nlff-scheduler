...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        WITH seed_orgs(name) AS (
            VALUES
              ('Lake County Stallions'), ('Cary'), ('Johnsburg'), ('Woodstock'), ('Westosha'), ('Antioch'), ('Prairie Ridge')
        )
        INSERT INTO organizations (id, name, is_active)
        SELECT gen_random_uuid(), name, true FROM seed_orgs
        ON CONFLICT (name) DO NOTHING;
        ...
        WITH seeded_users(organization_name, full_name, email, temporary_password) AS (
            VALUES
            ...
            JOIN organizations o ON o.name = su.organization_name
