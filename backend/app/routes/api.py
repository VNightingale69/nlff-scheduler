...
def _build_host_location_vs_home_team_verification(db: Session, season_id: uuid.UUID | str | None = None, *, require_host_owner_as_home_team: bool = False) -> dict[str, object]:
    ...
    if require_host_owner_as_home_team and counts['HOST_OWNER_IS_AWAY'] > 0:
        validation_failures.append('Host owner is away team for one or more games while require_host_owner_as_home_team is true.')
    elif counts['HOST_OWNER_IS_AWAY'] > 0:
        informational_notes.append('Host owner is away team for one or more games; require_host_owner_as_home_team is false.')
    if counts['NEUTRAL_SITE'] > 0:
        informational_notes.append('Neutral-site games were found.')

    return {
        'title': 'Host Location vs Home Team Verification',
        'require_host_owner_as_home_team': require_host_owner_as_home_team,
...
def _build_revised_scheduling_hierarchy_diagnostics(db: Session, season_id: uuid.UUID | str | None, *, compaction_diagnostics: dict[str, object] | None = None) -> dict[str, object]:
    """Return optimizer diagnostics aligned to the hard-rule-first scheduling hierarchy.

    This helper is reporting-only: it must not mutate placement state and it must
    never turn a diagnostics error into a placement failure.
    """
    diagnostics_warnings: list[str] = []
    try:
        rows = _schedule_management_rows(db, {'season_id': season_id}) if season_id else _schedule_management_rows(db)
    except Exception as exc:
        logger.exception('revised scheduling diagnostics failed to load scheduled rows season_id=%s', season_id)
        rows = []
        diagnostics_warnings.append(f'Diagnostics warning: scheduled-row diagnostics unavailable ({exc}).')

    games_committed = len(rows)
    games_required = games_committed
    try:
        if season_id:
            regular_weeks = db.query(Week).filter(Week.season_id == season_id).all()
            regular_weeks = [week for week in regular_weeks if _is_regular_season_week(week)]
            active_divisions = db.query(Division).filter(Division.is_active.is_(True)).all()
            games_required = 0
            for division in active_divisions:
                ...
                games_required += _division_required_games(active_team_count, division) * len(regular_weeks)
    except Exception as exc:
        diagnostics_warnings.append(f'Diagnostics warning: required-game count fell back to committed games ({exc}).')
        games_required = games_committed
    games_generated = games_committed
    ...
    matchup_seen: dict[tuple[uuid.UUID, uuid.UUID, uuid.UUID | None, uuid.UUID | None], int] = {}
    team_time_conflicts: list[dict[str, object]] = []
    field_time_conflicts: list[dict[str, object]] = []
    duplicate_games: list[dict[str, object]] = []
    invalid_field_size_assignments: list[dict[str, object]] = []
    invalid_wave_configuration_games: list[dict[str, object]] = []
    invalid_field_component_games: list[dict[str, object]] = []
    turf_wave_component_seen: dict[tuple[uuid.UUID | None, uuid.UUID | None], str] = {}
    host_owner_as_away_games: list[dict[str, object]] = []
    host_owner_as_home_count = 0
    same_community_games = 0
    neutral_site_games = 0
    true_home_possible = 0
    ...
    for g, slot, fi, host, home, away, div, org, status in rows:
        ...
        for team in (home, away):
            ...
            entry = {'game_id': str(g.id), 'team_id': str(team.id), 'team_name': team.name, 'date': g.game_date.isoformat(), 'start_time': g.kickoff_time.isoformat()}
            if key in team_time_seen:
                team_time_conflicts.append({'conflicting_games': [team_time_seen[key], entry]})
            else:
                team_time_seen[key] = entry
        field_key = (getattr(host, 'id', None), getattr(fi, 'id', None), g.game_date, g.kickoff_time)
        field_entry = {'game_id': str(g.id), 'host_location_id': str(host.id) if host else None, 'field_instance_id': str(fi.id) if fi else None, 'date': g.game_date.isoformat(), 'start_time': g.kickoff_time.isoformat()}
        ...
        if assigned_size and required_size != assigned_size:
            invalid_field_size_assignments.append({'game_id': str(g.id), 'division': f'{div.division_group or ""} {div.name or ""}'.strip(), 'required_field_size': required_size, 'assigned_field_size': assigned_size})
        if slot and slot.turf_wave_id:
            wave = slot.turf_wave
            selected_config = _normalize_configuration_name(getattr(wave, 'preferred_layout_code', None)) if wave else None
            expected_counts = _turf_wave_layout_counts(selected_config, [slot])
            if selected_config not in TURF_APPROVED_LAYOUT_CODES or not _is_approved_turf_slot_counts(expected_counts):
                invalid_wave_configuration_games.append({'game_id': str(g.id), 'wave_id': str(slot.turf_wave_id), 'selected_wave_configuration': selected_config})
            component_key = (slot.turf_wave_id, slot.field_instance_id)
            if component_key in turf_wave_component_seen:
                invalid_field_component_games.append({'game_id': str(g.id), 'conflicting_game_id': turf_wave_component_seen[component_key], 'wave_id': str(slot.turf_wave_id), 'field_instance_id': str(slot.field_instance_id) if slot.field_instance_id else None})
            else:
                turf_wave_component_seen[component_key] = str(g.id)
        host_owner_id = getattr(host, 'organization_id', None) if host else None
        home_org_id = getattr(home, 'organization_id', None)
        away_org_id = getattr(away, 'organization_id', None)
        same_community = bool(home_org_id and home_org_id == away_org_id)
        if same_community:
            same_community_games += 1
        true_home_candidate = bool(host_owner_id and host_owner_id in {home_org_id, away_org_id})
        if true_home_candidate:
            true_home_possible += 1
            if host_owner_id == home_org_id:
                true_home_scheduled += 1
                host_owner_as_home_count += 1
            elif host_owner_id == away_org_id:
                host_owner_as_away_games.append({'game_id': str(g.id), 'host_location_id': str(host.id) if host else None, 'host_location_name': host.name if host else None, 'away_team_id': str(away.id), 'away_team_name': away.name})
                true_home_not_scheduled.append({'game_id': str(g.id), 'reason': 'HOST_OWNER_WAS_AWAY_TEAM'})
        else:
            neutral_site_games += 1
        if host:
            hd = host_diag_by_id.setdefault(host.id, {
                'host_location_id': str(host.id),
                'host_location_name': host.name,
                'owner_community_id': str(host.organization_id) if host.organization_id else None,
                'games_involving_host_community': 0,
                'games_involving_host_community_scheduled_at_home': 0,
                'games_involving_host_community_scheduled_elsewhere': 0,
                'reason_scheduled_elsewhere': {},
            })
            if host.organization_id in {home_org_id, away_org_id}:
                hd['games_involving_host_community'] = int(hd['games_involving_host_community']) + 1
                if host.organization_id == home_org_id:
                    hd['games_involving_host_community_scheduled_at_home'] = int(hd['games_involving_host_community_scheduled_at_home']) + 1
                else:
                    hd['games_involving_host_community_scheduled_elsewhere'] = int(hd['games_involving_host_community_scheduled_elsewhere']) + 1
    ...
    schedule_health_summary = {
        ...
        'required_games_missing': max(games_required - games_committed, 0),
        'duplicate_games': len(duplicate_games),
        'duplicate_game_details': duplicate_games,
        ...
        'invalid_field_component_games': len(invalid_field_component_games),
        'rollback_occurred': bool((compaction_diagnostics or {}).get('rollback_occurred')),
        'rollback_reason': (compaction_diagnostics or {}).get('rollback_reason'),
    }
    ...
    for game_date in sorted(set([*games_by_date.keys(), *[key[0] for key in slots_by_date_host.keys()]])):
        ...
        primary_available = [str(hid) for hid in sorted(date_host_ids, key=str) if _classify_host_for_date(db, hosts_by_id.get(hid)) in {HOST_ROLE_PRIMARY_TURF, HOST_ROLE_PRIMARY_GRASS, HOST_ROLE_STANDARD_GRASS}]
        ...
        games_assigned_by_host_and_field_size: dict[str, dict[str, int]] = {}
        for g, slot, _fi, host, home, away, div, _org, _status in games_by_date.get(game_date, []):
            if not host:
                continue
            host_key = str(host.id)
            games_assigned_by_host[host_key] = games_assigned_by_host.get(host_key, 0) + 1
            size = _normalize_field_size(slot.field_type if slot else None) or _required_field_type_for_division(div)
            games_assigned_by_host_and_field_size.setdefault(host_key, _empty_field_size_counts())[size] += 1
            host_owner_id = getattr(host, 'organization_id', None)
            if host_owner_id not in {getattr(home, 'organization_id', None), getattr(away, 'organization_id', None)}:
                neutral_games_by_host[host_key] = neutral_games_by_host.get(host_key, 0) + 1
            if host_key in primary_available:
                primary_host_counts[host_key] = primary_host_counts.get(host_key, 0) + 1
        capacity_used_by_host: dict[str, dict[str, int]] = {}
        capacity_remaining_by_host: dict[str, dict[str, int]] = {}
        reason_by_host: dict[str, str] = {}
        for host_id in date_host_ids:
            slots = slots_by_date_host.get((game_date, host_id), [])
            total = _capacity_by_size_from_slots(slots)
            used = _capacity_by_size_from_slots([slot for slot in slots if slot.assigned_game_id])
            remaining = {size: max(int(total.get(size, 0) or 0) - int(used.get(size, 0) or 0), 0) for size in FIELD_SIZE_ORDER}
            capacity_used_by_host[str(host_id)] = used
            capacity_remaining_by_host[str(host_id)] = remaining
            reason_by_host[str(host_id)] = 'Used for scheduled games.' if host_id in active_host_ids else ('Available but not needed after primary/compact placement.' if sum(total.values()) else 'No compatible generated components available.')
            field_utilization_diagnostics.append({
                'scheduling_date': game_date.isoformat(),
                'host_location_id': str(host_id),
                'host_location_name': hosts_by_id.get(host_id).name if hosts_by_id.get(host_id) else None,
                'total_available_components_by_size': total,
                'used_components_by_size': used,
                'unused_components_by_size': remaining,
                'utilization_percentage': round((sum(used.values()) / max(sum(total.values()), 1)) * 100, 2),
                'compatible_games_scheduled_later_while_component_unused': False,
        ...
        active_host_locations_by_surface_type: dict[str, list[str]] = {}
        host_location_owner_community_id = {}
        for host in active_host_location_rows:
            surface = str(host.surface_type or 'UNKNOWN')
            active_host_locations_by_surface_type.setdefault(surface, []).append(str(host.id))
            host_location_owner_community_id[str(host.id)] = str(host.organization_id) if host.organization_id else None
        hosting_community_game_rows: list[dict[str, object]] = []
        hosting_community_team_ids_with_games: set[str] = set()
        scheduled_at_owned_total = 0
        scheduled_elsewhere_total = 0
        active_host_location_details: list[dict[str, object]] = []
        for active_host in active_host_location_rows:
            owner_id = active_host.organization_id
            games_involving_owner = 0
            owner_here = 0
            owner_elsewhere = 0
            neutral_here = 0
            elsewhere_reasons: dict[str, int] = {}
            for g, slot, _fi, actual_host, home, away, div, _org, _status in games_by_date.get(game_date, []):
                home_org = getattr(home, 'organization_id', None)
                away_org = getattr(away, 'organization_id', None)
                actual_host_id = getattr(actual_host, 'id', None)
                if actual_host_id == active_host.id and owner_id not in {home_org, away_org}:
                    neutral_here += 1
                if owner_id in {home_org, away_org}:
                    games_involving_owner += 1
                    if actual_host_id == active_host.id:
                        owner_here += 1
                    else:
                        owner_elsewhere += 1
                        reason = 'scheduled at another active owned host location' if actual_host_id in active_host_ids_by_org.get(owner_id, set()) else 'no hard exception reason recorded'
                        elsewhere_reasons[reason] = elsewhere_reasons.get(reason, 0) + 1
            active_host_location_details.append({
                'host_location_id': str(active_host.id),
                'host_location_name': active_host.name,
                'surface_type': active_host.surface_type,
                'owner_community_id': str(owner_id) if owner_id else None,
                'games_involving_owner_community': games_involving_owner,
                'owner_community_games_scheduled_here': owner_here,
                'owner_community_games_scheduled_elsewhere': owner_elsewhere,
                'neutral_site_games_scheduled_here': neutral_here,
                'reason_owner_community_games_scheduled_elsewhere': elsewhere_reasons,
            })
        for g, slot, _fi, actual_host, home, away, div, _org, _status in games_by_date.get(game_date, []):
            for team in (home, away):
                hosting_org_id = getattr(team, 'organization_id', None)
                if hosting_org_id not in active_hosting_community_ids:
                    continue
                hosting_community_team_ids_with_games.add(str(team.id))
                ...
                actual_host_id = getattr(actual_host, 'id', None)
                scheduled_at_owned = bool(actual_host_id in expected_host_ids)
                ...
                compatible_owned_slots = [
                    owned_slot
                    for date_host_slots in slots_by_date_host.values()
                    for owned_slot in date_host_slots
                    if owned_slot.slot_date == game_date
                    and owned_slot.host_location_id in expected_host_ids
                    and _normalize_field_size(owned_slot.field_type) == _normalize_field_size(required_size)
                ]
                exception_reason = None
                rejection_reasons: dict[str, int] = {}
                if not scheduled_at_owned:
                    if compatible_owned_slots:
                        exception_reason = 'Compatible owned host slots existed but were unavailable after hard constraints such as team conflicts, field conflicts, doubleheader preservation, or prior assigned capacity.'
                        rejection_reasons['compatible owned host slots unavailable after hard constraints'] = len(compatible_owned_slots)
                    else:
                        exception_reason = 'No compatible active owned host slot was available for the required field size/date.'
                        rejection_reasons['invalid field size or no generated owned host slots for required field size'] = 1
                if scheduled_at_owned:
                    scheduled_at_owned_total += 1
                else:
                    scheduled_elsewhere_total += 1
                if not scheduled_at_owned and not exception_reason:
                    bug_warnings.append('BUG: hosting-community team was not scheduled at its own active host location and no hard exception reason was recorded.')
                if scheduled_at_owned and not team_is_home and not same_hosting_community:
                    bug_warnings.append('BUG: hosting-community team playing at own active host location but not assigned as home team.')
                hosting_community_game_rows.append({
                    ...
                    'exception_used': bool(exception_reason),
                    'exception_reason': exception_reason,
                    'home_host_slots_considered': len(compatible_owned_slots),
                    'rejection_reasons_for_home_host_slots': rejection_reasons,
                })
        total_hosting_rows = len(hosting_community_game_rows)
        true_home_host_enforcement_by_date.append({
            'scheduling_date': game_date.isoformat(),
            'active_hosting_communities': [str(org_id) for org_id in sorted(active_hosting_community_ids, key=str)],
            'active_host_locations': [str(host.id) for host in active_host_location_rows],
            'active_host_locations_by_surface_type': active_host_locations_by_surface_type,
            'host_location_owner_community_id': host_location_owner_community_id,
            'hosting_community_teams_with_games': sorted(hosting_community_team_ids_with_games),
            'hosting_community_games_total': total_hosting_rows,
            'hosting_community_games_scheduled_at_owned_host_location': scheduled_at_owned_total,
            'hosting_community_games_scheduled_elsewhere': scheduled_elsewhere_total,
            'hosting_community_home_host_success_rate': round((scheduled_at_owned_total / max(total_hosting_rows, 1)) * 100, 2),
            'host_owner_as_away_count': len([row for row in host_owner_as_away_games if any(row.get('game_id') == str(g.id) for g, *_rest in games_by_date.get(game_date, []))]),
            'hosting_community_games': hosting_community_game_rows,
            'active_host_location_diagnostics': active_host_location_details,
        })
        host_activation_summary_by_date.append({
            'scheduling_date': game_date.isoformat(),
            'available_hosts': [str(hid) for hid in sorted(date_host_ids, key=str)],
            'active_hosts': [str(hid) for hid in sorted(active_host_ids, key=str)],
            'primary_hosts_available': primary_available,
            'primary_hosts_used': [hid for hid in primary_available if uuid.UUID(hid) in active_host_ids],
            'overflow_hosts_available': overflow_available,
            'overflow_hosts_used': overflow_used,
            'overflow_required': bool(overflow_used),
            'overflow_required_reason': 'Overflow host has scheduled games.' if overflow_used else None,
            'overflow_avoided': bool(overflow_available and not overflow_used),
            'overflow_avoided_reason': 'Primary hosts absorbed scheduled compatible demand.' if overflow_available and not overflow_used else None,
            'games_assigned_by_host': games_assigned_by_host,
            'games_assigned_by_host_and_field_size': games_assigned_by_host_and_field_size,
            'capacity_used_by_host': capacity_used_by_host,
            'capacity_remaining_by_host': capacity_remaining_by_host,
            'reason_each_available_host_was_used_or_not_used': reason_by_host,
        })

    start_times = sorted({time_value for values in starts_by_date.values() for time_value in values})
    earliest_start = start_times[0].isoformat() if start_times else None
    latest_start = start_times[-1].isoformat() if start_times else None
    total_window = ((_minutes_from_time(start_times[-1]) or 0) - (_minutes_from_time(start_times[0]) or 0) + GAME_DURATION_MINUTES) if len(start_times) >= 2 else (GAME_DURATION_MINUTES if start_times else 0)
    games_by_start_time: dict[str, int] = {}
    ...
    for g, _slot, fi, host, _home, _away, div, _org, _status in rows:
        ...
        if fi:
            fields_used_by_start_time[key] = fields_used_by_start_time.get(key, 0) + 1
        if (_minutes_from_time(g.kickoff_time) or 0) >= 15 * 60:
            late_games.append({
                'game_id': str(g.id),
                'division': f'{div.division_group or ""} {div.name or ""}'.strip(),
                'current_start_time': g.kickoff_time.isoformat(),
                'current_host': host.name if host else None,
                'current_field': fi.field_name if fi else None,
                'earlier_slots_considered': [],
                'earlier_slot_rejection_reasons': {},
                'final_reason_game_remained_late': 'Late games are allowed; no positive-net earlier move was committed by the hard-constraint-safe optimizer.',
            })

    neutral_counts = list(neutral_games_by_host.values())
    primary_counts = list(primary_host_counts.values())
    neutral_balance = (max(neutral_counts) - min(neutral_counts)) if neutral_counts else 0
    primary_balance = (max(primary_counts) - min(primary_counts)) if primary_counts else 0
    neutral_rejected = dict((compaction_diagnostics or {}).get('neutral_site_to_neutral_site_rejected_by_reason') or {})
    ...
    for wave in waves:
        ...
        unused = {size: max(int(total.get(size, 0) or 0) - int(used.get(size, 0) or 0), 0) for size in FIELD_SIZE_ORDER}
        ...
        row = {
            ...
            'approved_configurations_considered': [],
            'selected_configuration_reason': 'Generated turf wave configuration selected during slot generation.',
            'expected_components_by_size': total,
            'assigned_components_by_size': used,
            'unused_components_by_size': unused,
            'utilization_percentage': round((used_components / max(total_components, 1)) * 100, 2),
            'status': status,
            'games_assigned_to_wave': [str(slot.assigned_game_id) for slot in slots if slot.assigned_game_id],
            'generated_field_components': [{'field_instance_id': str(slot.field_instance_id) if slot.field_instance_id else None, 'field_component': slot.field_instance.field_name if slot.field_instance else None, 'field_size': _normalize_field_size(slot.field_type)} for slot in slots],
        }
        turf_wave_rows.append(row)
        date_key = row['game_date'] or 'unknown'
        summary = turf_summary_by_date.setdefault(date_key, {'scheduling_date': date_key, 'total_turf_waves': 0, 'full_turf_waves': 0, 'partial_turf_waves': 0, 'empty_turf_waves': 0, 'partial_waves_by_configuration': {}, 'unused_small_components': 0, 'unused_medium_components': 0, 'unused_large_components': 0})
        summary['total_turf_waves'] += 1
        summary[f'{status.lower()}_turf_waves'] += 1
        if status == 'PARTIAL':
            cfg = row['selected_wave_configuration'] or 'UNKNOWN'
            summary['partial_waves_by_configuration'][cfg] = summary['partial_waves_by_configuration'].get(cfg, 0) + 1
        summary['unused_small_components'] += unused.get(FIELD_SIZE_SMALL, 0)
    ...
    return {
        ...
        'true_home_host_diagnostics': {
            ...
            'true_home_host_games_scheduled': true_home_scheduled,
            'true_home_host_games_not_scheduled': len(true_home_not_scheduled),
            'reason_true_home_host_game_not_scheduled': true_home_not_scheduled,
            'host_owner_as_home_count': host_owner_as_home_count,
            'host_owner_as_away_count': len(host_owner_as_away_games),
        ...
        'game_day_compaction_diagnostics': {
            ...
            'unused_compatible_components_by_start_time': {},
            'late_games_before_optimization': len(late_games),
            'late_games_after_optimization': len(late_games),
            'games_moved_earlier': int((compaction_diagnostics or {}).get('accepted_moves_count') or 0),
        ...
        'pull_forward_optimization_diagnostics': {
            ...
            'candidate_games_considered': int((compaction_diagnostics or {}).get('candidate_moves_evaluated') or 0),
            'candidates_accepted': int((compaction_diagnostics or {}).get('accepted_moves_count') or 0),
            'candidates_rejected': int((compaction_diagnostics or {}).get('rejected_moves_count') or 0),
            'candidates_rejected_by_reason': dict((compaction_diagnostics or {}).get('rejected_moves_by_reason') or {}),
            'games_moved_earlier': (compaction_diagnostics or {}).get('accepted_moves') or [],
            'latest_start_before': (compaction_diagnostics or {}).get('latest_start_before'),
...
def _diagnostic_no_bye_doubleheaders_enabled(division: Division | None = None, season: Season | None = None) -> bool:
    """Return the diagnostics view of odd-team no-bye scheduling.

    The current auto-scheduler defaults to no-bye odd divisions unless a future
    season/division setting explicitly opts out. Keep this helper diagnostic-only
    so expected counts can follow those settings when they are introduced without
    changing scheduling behavior here.
    """
    setting_names = (
        'no_byes',
        'no_bye_scheduling_enabled',
        'odd_team_no_bye_doubleheaders_enabled',
        'enable_odd_team_doubleheaders',
    )
    for source in (division, season):
        if source is None:
            continue
        for name in setting_names:
            if hasattr(source, name):
                return bool(getattr(source, name))
        for name in ('bye_scheduling_enabled', 'allow_byes'):
            if hasattr(source, name):
                return not bool(getattr(source, name))
    return True
...
def _diagnostic_active_teams_expected_to_play(active_team_count: int | None, *, no_bye_doubleheaders_enabled: bool = True) -> int:
    team_count = max(int(active_team_count or 0), 0)
    if team_count % 2 == 0 or no_bye_doubleheaders_enabled:
        return team_count
    return max(0, team_count - 1)


def _diagnostic_field_size_diff(expected: dict[str, int], actual: dict[str, int]) -> tuple[dict[str, int], dict[str, int]]:
    missing = {size: max(0, int(expected.get(size, 0) or 0) - int(actual.get(size, 0) or 0)) for size in FIELD_SIZE_ORDER}
    extra = {size: max(0, int(actual.get(size, 0) or 0) - int(expected.get(size, 0) or 0)) for size in FIELD_SIZE_ORDER}
...
def _select_turf_wave_configuration(remaining_demand: dict[str, int], active_configuration_names: set[str], *, wave_index: int = 0) -> str | None:
    ...
    if not candidates:
        return None
    return max(candidates, key=lambda name: _score_turf_wave_configuration(name, remaining_demand, wave_index=wave_index))
...
def _plan_turf_layout_blocks(
    ...
    """Select one approved turf configuration independently for each time-block wave.

    The return shape is kept as (layout, hours) for callers, but every generated
    item is a one-hour wave.  This avoids treating a turf layout as static across
    a day and lets overflow decisions inspect true per-wave capacity.
    """
    if not any(demand_counts.get(size, 0) > 0 for size in FIELD_SIZE_ORDER):
        return []
    remaining = _normalized_demand_counts(demand_counts)
    blocks: list[tuple[str, int]] = []
    for wave_index in range(max(int(total_hours or 0), 0)):
        selected = _select_turf_wave_configuration(remaining, active_configuration_names, wave_index=wave_index)
        if not selected:
            break
        blocks.append((selected, 1))
        metadata = _turf_configuration_metadata(selected) or {'counts': {}}
        for size in FIELD_SIZE_ORDER:
            remaining[size] = max(0, int(remaining.get(size, 0) or 0) - int(metadata['counts'].get(size, 0) or 0))
    return blocks
...
HOST_ROLE_STANDARD_GRASS = 'STANDARD_GRASS'
HOST_ROLE_UNAVAILABLE = 'UNAVAILABLE'


def _host_role_value(host: HostLocation | None) -> str | None:
    role = str(getattr(host, 'host_role', '') or '').strip().upper() if host else ''
    return role or None
...
def _host_supports_turf_waves(db: Session, host: HostLocation | None) -> bool:
    ...
    _ensure_approved_turf_configurations(db, host)
    db.flush()
    active_count = db.query(HostLocationConfiguration.id).filter(
        HostLocationConfiguration.host_location_id == host.id,
        HostLocationConfiguration.is_active.is_(True),
    ).count()
...
def _classify_host_for_date(db: Session, host: HostLocation, availability: HostingAvailability | None = None) -> str:
    if not host or not host.is_active or (availability is not None and not availability.is_available):
        return HOST_ROLE_UNAVAILABLE
    configured_role = _host_role_value(host)
    if configured_role in {HOST_ROLE_PRIMARY_TURF, HOST_ROLE_PRIMARY_GRASS, HOST_ROLE_OVERFLOW_GRASS, HOST_ROLE_STANDARD_GRASS}:
        return configured_role
    surface = str(host.surface_type or 'GRASS_FIELD').upper()
    if surface == 'TURF_STADIUM' and _host_supports_turf_waves(db, host):
        return HOST_ROLE_PRIMARY_TURF
    if surface == 'GRASS_FIELD':
        sibling_turf_exists = db.query(HostLocation.id).filter(
            HostLocation.organization_id == host.organization_id,
            HostLocation.id != host.id,
            HostLocation.is_active.is_(True),
            HostLocation.surface_type == 'TURF_STADIUM',
        ).first() is not None
        return HOST_ROLE_OVERFLOW_GRASS if sibling_turf_exists else HOST_ROLE_STANDARD_GRASS
    return HOST_ROLE_STANDARD_GRASS
...
def _date_host_availabilities(db: Session, season_id: uuid.UUID | str | None, week_id: uuid.UUID | str | None, game_date: date | None, allowed_host_ids: set[uuid.UUID] | None = None) -> list[tuple[HostLocation, HostingAvailability]]:
    if not season_id or not week_id or not game_date:
        return []
    rows = db.query(HostLocation, HostingAvailability).join(
        HostingAvailability, HostingAvailability.host_location_id == HostLocation.id
    ).filter(
        HostingAvailability.season_id == season_id,
        HostingAvailability.week_id == week_id,
        HostingAvailability.primary_game_date == game_date,
    ...
    if allowed_host_ids is not None:
        rows = [(host, availability) for host, availability in rows if host.id in allowed_host_ids]
    return rows


def _dynamic_turf_capacity_for_availability(db: Session, host: HostLocation, availability: HostingAvailability, demand_by_size: dict[str, int]) -> tuple[dict[str, int], list[dict[str, object]]]:
    _ensure_approved_turf_configurations(db, host)
    db.flush()
    active_configs = db.query(HostLocationConfiguration).filter(
        HostLocationConfiguration.host_location_id == host.id,
    ...
    active_names = _available_turf_configuration_names(active_configs)
    hours = _hours_between(availability.start_time, availability.end_time, availability.available_date)
    remaining = _normalized_demand_counts(demand_by_size)
    capacity = _empty_field_size_counts()
...
def _capacity_by_size_from_slots(slots: list[GameSlot]) -> dict[str, int]:
    counts = _empty_field_size_counts()
    for slot in slots:
        size = _normalize_field_size(slot.field_type)
        if size in counts:
...
def _host_activation_plan_for_date(db: Session, season_id: uuid.UUID | str | None, week_id: uuid.UUID | str | None, game_date: date | None, demand_by_size: dict[str, int], allowed_host_ids: set[uuid.UUID] | None = None) -> dict[str, object]:
    ...
    classified: dict[str, list[tuple[HostLocation, HostingAvailability]]] = {
        HOST_ROLE_PRIMARY_TURF: [], HOST_ROLE_PRIMARY_GRASS: [], HOST_ROLE_OVERFLOW_GRASS: [], HOST_ROLE_STANDARD_GRASS: [], HOST_ROLE_UNAVAILABLE: []
    }
    ...
    primary_grass_capacity = _empty_field_size_counts()
    primary_grass_host_capacity_diagnostics: list[dict[str, object]] = []
    for host, availability in classified.get(HOST_ROLE_PRIMARY_GRASS, []):
        slots = db.query(GameSlot).filter(
            GameSlot.host_location_id == host.id,
            GameSlot.slot_date == game_date,
            GameSlot.start_time >= availability.start_time,
            GameSlot.end_time <= availability.end_time,
        ).all()
        capacity = _capacity_by_size_from_slots(slots)
        for size in FIELD_SIZE_ORDER:
            primary_grass_capacity[size] += int(capacity.get(size, 0) or 0)
        host_can_fit, host_reasons = _host_plan_capacity_sufficiency(capacity, demand_by_size)
        primary_grass_host_capacity_diagnostics.append({
            'host_location_id': str(host.id),
            'host_location_name': host.name,
            'availability_id': str(availability.id),
            'available_window': {
            ...
            'demand_by_field_size': _normalized_demand_counts(demand_by_size),
            'generated_slot_capacity_by_field_size': capacity,
    ...
    primary_host_ids = primary_turf_ids | primary_grass_ids
    ...
    if primary_host_ids:
        ...
        if primary_can_fit:
            ...
            activation_reason = 'PRIMARY_HOST_COMPATIBLE_CAPACITY_INSUFFICIENT: ' + '; '.join(primary_reasons)
            selected_host_ids.update(overflow_grass_ids)
            activated_overflow_ids.update(overflow_grass_ids)
    else:
    ...
    for row in overflow_grass_host_diagnostics:
        ...
        elif primary_host_ids and primary_can_fit:
            row['activated'] = False
            row['activation_decision_reason'] = 'Primary host compatible capacity can fit all required games; overflow grass host not needed.'
        elif primary_host_ids:
            row['activated'] = False
            row['activation_decision_reason'] = 'Primary host capacity was insufficient, but this host was not selected for activation.'
        else:
            row['activated'] = host_id in selected_host_ids
            row['activation_decision_reason'] = 'No primary host was available; standard/overflow grass hosts were used by fallback selection.'
    return {
        ...
        'primary_hosts_available': [str(host.id) for host, _ in classified.get(HOST_ROLE_PRIMARY_TURF, []) + classified.get(HOST_ROLE_PRIMARY_GRASS, [])],
        ...
        'primary_turf_capacity_diagnostics': {
            ...
            'primary_grass_hosts': primary_grass_host_capacity_diagnostics,
        },
        'overflow_grass_hosts_considered': [str(host.id) for host, _ in classified.get(HOST_ROLE_OVERFLOW_GRASS, [])],
        'overflow_hosts_available': [str(host.id) for host, _ in classified.get(HOST_ROLE_OVERFLOW_GRASS, [])],
        'overflow_hosts_used': [str(host_id) for host_id in sorted(activated_overflow_ids, key=str)],
        'overflow_required': bool(activated_overflow_ids),
        'overflow_required_reason': activation_reason if activated_overflow_ids else None,
        'overflow_avoided': bool(primary_host_ids and primary_can_fit and not activated_overflow_ids),
        'overflow_avoided_reason': 'Primary host compatible capacity absorbed demand.' if primary_host_ids and primary_can_fit and not activated_overflow_ids else None,
        'overflow_grass_host_diagnostics': {
            'diagnostic_label': 'Overflow Grass Host Activation',
            'activation_reason': activation_reason,
            'primary_host_capacity_can_fit_all_required_games': primary_can_fit,
            'primary_host_capacity_shortfall_reasons': primary_reasons,
        ...
        'overflow_hosts_activated': [str(host_id) for host_id in sorted(activated_overflow_ids, key=str)],
        'overflow_host_activation_reason': activation_reason,
        'primary_turf_capacity_used': _empty_field_size_counts(),
        'primary_turf_capacity_remaining': primary_capacity,
        'primary_grass_capacity_remaining': primary_grass_capacity,
        'primary_combined_capacity_remaining': primary_combined_capacity,
        'minimum_games_to_activate_overflow_host': minimum_threshold,
        'overflow_host_below_threshold': False,
        'turf_wave_configuration_candidates_evaluated': sum(len(row.get('approved_configurations_considered') or []) for row in wave_definitions),
        'selected_wave_configurations_by_time': wave_definitions,
        'turf_wave_definition_and_utilization_summary': wave_definitions,
    }
...
def _run_turf_wave_compaction_pass(db: Session, season_id: uuid.UUID, *, enabled: bool = True) -> dict[str, object]:
    diagnostics: dict[str, object] = {
        'section': 'Turf Wave Compaction Rejection Diagnostics',
        'legacy_section': 'Turf Wave Compaction',
        'compaction_enabled': bool(enabled),
        'compaction_pass_started': False,
        'compaction_pass_completed': False,
        'pull_forward_started': False,
        'pull_forward_completed': False,
        'candidate_games_considered': 0,
        'candidates_accepted': 0,
        'candidates_rejected': 0,
        'candidates_rejected_by_reason': {},
        'games_moved_earlier': [],
        'latest_start_before': None,
        'latest_start_after': None,
        'active_time_window_before': None,
        'active_time_window_after': None,
        'primary_host_balance_before': None,
        'primary_host_balance_after': None,
        'field_utilization_before': None,
        'field_utilization_after': None,
    ...
    if not enabled:
        ...
        diagnostics['rejected_moves_by_reason'] = {'COMPACTION_DISABLED': 1}
        logger.info(
            'turf_wave_compaction_skipped season_id=%s enabled=%s reason=%s',
            season_id,
            enabled,
            diagnostics['compaction_skipped_reason'],
        )
        return diagnostics
    diagnostics['compaction_pass_started'] = True
    diagnostics['pull_forward_started'] = True
    logger.info('turf_wave_compaction_start season_id=%s enabled=%s', season_id, enabled)
    ...
    def _component_rows(counts: dict[str, int]) -> list[dict[str, object]]:
        return [
            {'field_type': size, 'count': count}
            for size in FIELD_SIZE_ORDER
            if (count := int(counts.get(size) or 0)) > 0
        ]

    def _field_size_label(value: object) -> str | None:
        normalized = _normalize_field_size(value)
        return normalized if normalized in FIELD_SIZE_ORDER else None

    def _wave_utilization_snapshot(wave: TurfWave | None, slots: list[GameSlot] | None = None, *, extra_assigned_delta: int = 0) -> dict[str, object] | None:
        if not wave:
            return None
        if slots is None:
            slots = db.query(GameSlot).filter(GameSlot.turf_wave_id == wave.id).all()
        ...
        return {
            ...
            'assigned_components': total_used,
            'capacity_components': total_capacity,
    ...
    def _team_time_conflict_detail(game: Game, target: GameSlot) -> str:
        team_ids = {game.home_team_id, game.away_team_id}
        conflict = db.query(Game).join(Game.status).filter(
            Game.season_id == season_id,
            Game.id != game.id,
            GameStatus.code != 'UNSCHEDULED',
            Game.game_date == target.slot_date,
        ...
        if conflict and conflict.home_team and conflict.home_team_id in team_ids:
            team_name = conflict.home_team.name
        elif conflict and conflict.away_team and conflict.away_team_id in team_ids:
            team_name = conflict.away_team.name
        else:
            team_name = _game_teams(game) or 'A selected team'
        return f'{team_name} already plays at {target.start_time.isoformat()} on {target.slot_date.isoformat()}.'

    def _doubleheader_detail(game: Game, source: GameSlot, target: GameSlot) -> str:
        for team_id in (game.home_team_id, game.away_team_id):
            games = db.query(Game).join(Game.status).filter(
                Game.season_id == season_id,
                GameStatus.code != 'UNSCHEDULED',
            ...
            if len(games) <= 1:
                continue
            before = sorted(row.kickoff_time for row in games if row.kickoff_time)
            after = sorted(target.start_time if row.id == game.id else row.kickoff_time for row in games if row.kickoff_time)
            if before != after:
                team = db.get(Team, team_id)
                return f'Moving this game to {target.start_time.isoformat()} would separate the selected doubleheader team {team.name if team else team_id} games from {"/".join(t.isoformat() for t in before)} to {"/".join(t.isoformat() for t in after)}.'
        return f'Moving this game to {target.start_time.isoformat()} would break selected doubleheader spacing.'

    def _score_detail(score_before: int | None, score_after: int | None, reason: str, source: GameSlot | None, target: GameSlot | None, target_wave: TurfWave | None = None) -> str:
        if reason == 'NO_NET_WAVE_IMPROVEMENT' and source and target:
            return 'Target wave improves by filling an unused component, but source wave utilization declines or the move does not improve the net active/filled wave score.'
        return reason.replace('_', ' ').lower()

    def _reject(
        game: Game | None,
        source: GameSlot | None,
        target: GameSlot | None,
        reason: str,
        *,
        detail: str | None = None,
        hard_constraint_failure: bool = True,
        ...
        _record_rejection_reason(normalized_reason)
        source_wave = source.turf_wave if source else None
        actual_target_wave = target_wave or (target.turf_wave if target else None)
        net_score_delta = (score_after - score_before) if score_before is not None and score_after is not None else None
        rejected_row = {
            'candidate_game_id': str(game.id) if game else None,
            'game_id': str(game.id) if game else None,
            'division': _division_label_for_game(game),
            ...
            'teams': _game_teams(game),
            'current_date': source.slot_date.isoformat() if source and source.slot_date else None,
            'current_host_location': source.host_location.name if source and source.host_location else None,
            'current_start_time': source.start_time.isoformat() if source and source.start_time else None,
            'current_field': _slot_label(source),
            'current_wave': _wave_label(source_wave),
            'original_host': source.host_location.name if source and source.host_location else None,
            'original_time': source.start_time.isoformat() if source and source.start_time else None,
            'original_field': _slot_label(source),
            'original_wave': _wave_label(source_wave),
            'target_host_location': target.host_location.name if target and target.host_location else None,
            'target_start_time': target.start_time.isoformat() if target and target.start_time else None,
            'target_field': _slot_label(target),
            'target_wave': _wave_label(actual_target_wave),
            'target_layout': _normalize_configuration_name(actual_target_wave.preferred_layout_code) if actual_target_wave else None,
            'new_host': target.host_location.name if target and target.host_location else None,
            'new_time': target.start_time.isoformat() if target and target.start_time else None,
            'new_field': _slot_label(target),
            'new_wave': _wave_label(actual_target_wave),
            'needed_field_size': _field_size_label(target.field_type) if target else None,
            'rejection_reason': normalized_reason,
            'reason': normalized_reason,
            'rejection_detail': detail or _score_detail(score_before, score_after, normalized_reason, source, target, actual_target_wave),
            'hard_constraint_failure': bool(hard_constraint_failure),
            'score_before': score_before,
            'score_after': score_after,
            'net_score_delta': net_score_delta,
            ...
            'target_wave_utilization_before': _wave_utilization_snapshot(actual_target_wave) if actual_target_wave else None,
            'target_wave_utilization_after': _wave_utilization_snapshot(actual_target_wave, extra_assigned_delta=1) if actual_target_wave else None,
    ...
    def _score_move(game: Game, source: GameSlot, target: GameSlot, target_wave: TurfWave, source_wave: TurfWave | None) -> tuple[int, list[str], dict[str, object] | None, dict[str, object] | None]:
        ...
        source_owner_id = source.host_location.organization_id if source and source.host_location else None
        target_owner_id = target.host_location.organization_id if target and target.host_location else None
        home_org_id = game.home_team.organization_id if game and game.home_team else None
        ...
        if source_is_true_home_host and target_owner_id != source_owner_id:
            active_owned_target_ids = {
                row[0]
                for row in db.query(GameSlot.host_location_id)
                .join(HostLocation, HostLocation.id == GameSlot.host_location_id)
                .filter(
                    GameSlot.season_id == season_id,
                    GameSlot.slot_date == target.slot_date,
                    HostLocation.organization_id == source_owner_id,
                )
                .distinct()
                .all()
                if row[0]
            ...
            if source.host_location_id in active_owned_target_ids:
                ...
                return reject_move('WOULD_BREAK_TRUE_HOME_HOST_PLACEMENT', detail='Optimization move would remove a hosting-community team from its active owned host location for compaction/utilization rather than to avoid a hard validation failure.', hard_constraint_failure=False)
        ...
        logger.debug(
            ...
            score,
            reasons,
            _wave_utilization_snapshot(source_wave) if source_wave else None,
            _wave_utilization_snapshot(source_wave, extra_assigned_delta=-1) if source_wave else None,
            _wave_utilization_snapshot(target_wave),
    ...
    def _logical_wave_group_key(wave: TurfWave) -> tuple[date | None, uuid.UUID | None, time | None, int | None, str]:
        return (
            wave.host_date,
            wave.host_location_id,
            wave.start_time,
    ...
    def _candidate_game_field_size(game: Game, source_slot: GameSlot | None) -> str | None:
        ...
        slot_field_size = _normalize_field_size(source_slot.field_type if source_slot else None)
        if slot_field_size:
            return slot_field_size
        slot_instance_size = _normalize_field_size(source_slot.field_instance.field_type if source_slot and source_slot.field_instance else None)
        if slot_instance_size:
            return slot_instance_size
        return _normalize_field_size(_game_required_field_type(game))

    def _record_candidate_evaluated() -> None:
    ...
    while int(diagnostics['moves_accepted']) < max_moves and target_partial_waves:
        ...
        for target_detail in target_partial_waves:
            ...
            target_waves: list[TurfWave] = []
            for wave_id in wave_ids:
                try:
                    parsed_wave_id = uuid.UUID(str(wave_id))
                except (TypeError, ValueError):
                    continue
            ...
            layout = str(target_detail.get('wave_layout') or _normalize_configuration_name(primary_target_wave.preferred_layout_code))
            target_slots = [slot for wave in target_waves for slot in slots_by_wave.get(wave.id, [])]
            counts = _assigned_counts(target_slots)
            capacity = _wave_capacity_counts(primary_target_wave, target_slots)
            ...
            if target_detail is not None:
                ...
                target_detail['same_date_matching_field_size_games_found'] = same_date_needed_count
                diagnostics['same_date_matching_field_size_games_found'] = int(diagnostics.get('same_date_matching_field_size_games_found') or 0) + same_date_needed_count
                target_detail['small_games_exist_elsewhere_same_date'] = bool(same_date_games_by_size.get(FIELD_SIZE_SMALL))
                target_detail['medium_games_exist_elsewhere_same_date'] = bool(same_date_games_by_size.get(FIELD_SIZE_MEDIUM))
                target_detail['large_games_exist_elsewhere_same_date'] = bool(same_date_games_by_size.get(FIELD_SIZE_LARGE))
                    ...
                    for game, source in same_date_games_by_size.get(target_size, []):
                        ...
                        if target_detail is not None and int(diagnostics.get('rejected_moves_count') or 0) > rejected_before:
                            ...
                            if int(diagnostics.get('candidates_rejected_by_hard_constraint') or 0) > hard_rejected_before:
                                target_detail['candidates_rejected_by_hard_constraint'] = int(target_detail.get('candidates_rejected_by_hard_constraint') or 0) + 1
    ...
    for row in partial_wave_detail_by_id.values():
        if int(row.get('same_date_matching_field_size_games_found') or row.get('same_date_needed_field_size_games_found_count') or 0) == 0:
    ...
    diagnostics['pull_forward_completed'] = True
    diagnostics['accepted_moves_count'] = int(diagnostics.get('moves_accepted') or 0)
    diagnostics['rejected_moves_count'] = int(diagnostics.get('moves_rejected') or 0)
    diagnostics['candidate_moves_evaluated'] = int(diagnostics.get('moves_evaluated') or 0)
    diagnostics['total_candidate_moves_evaluated'] = int(diagnostics.get('moves_evaluated') or 0)
    diagnostics['candidate_games_considered'] = int(diagnostics.get('moves_evaluated') or 0)
    diagnostics['candidates_accepted'] = int(diagnostics.get('moves_accepted') or 0)
    diagnostics['candidates_rejected'] = int(diagnostics.get('moves_rejected') or 0)
    diagnostics['candidates_rejected_by_reason'] = dict(diagnostics.get('rejected_moves_by_reason') or {})
    diagnostics['games_moved_earlier'] = list(diagnostics.get('accepted_moves') or [])
    diagnostics['waves_improved_count'] = int(diagnostics.get('moves_accepted') or 0)
    logger.info(
        'turf_wave_compaction_complete season_id=%s partial_waves=%s candidates_evaluated=%s accepted=%s rejected=%s top_rejection_reasons=%s rollback=%s',
        season_id,
        diagnostics.get('total_partial_waves_found'),
        diagnostics.get('total_candidate_moves_evaluated'),
        diagnostics.get('accepted_moves_count'),
        diagnostics.get('rejected_moves_count'),
        sorted((diagnostics.get('rejected_moves_by_reason') or {}).items(), key=lambda item: int(item[1]), reverse=True)[:5],
        diagnostics.get('rollback_occurred'),
    )
    return diagnostics
...
def auto_fill_preview(payload: dict, db: Session = Depends(get_db)):
    ...
    host_activation_summary = _host_activation_plan_for_date(db, season_id, week_id, week_game_date, weekly_demand_by_size, host_plan_allowed_ids)
    selected_host_ids: set[uuid.UUID] = set(host_activation_summary.get('selected_host_uuid_set') or set())
    overflow_host_ids: set[uuid.UUID] = set()
    ...
    if selected_host_ids and (host_activation_summary.get('primary_hosts_available') or host_activation_summary.get('primary_turf_hosts_considered')):
        locked_host_mode = 'primary_turf_dynamic_capacity'
        host_lock_reason = str(host_activation_summary.get('overflow_host_activation_reason') or 'primary turf host activation plan applied')
        overflow_host_ids.update({uuid.UUID(host_id) for host_id in (host_activation_summary.get('overflow_hosts_activated') or [])})
    elif host_capacity and community_rotation_ranking and host_plan_allowed_ids is not None:
    ...
    while remaining_slots and len(plans) < max_new_games:
            ...
            for slot in sorted_slots:
                ...
                for i in range(len(available_team_ids)):
                    for j in range(i + 1, len(available_team_ids)):
                        ...
                        true_home_host_candidate = bool(candidate_host_id and candidate_host_id in active_owned_participant_host_ids)
                        neutral_site_candidate = bool(host_org_id and host_org_id not in {team_a.organization_id, team_b.organization_id})
                        hosting_community_game = bool(active_owned_participant_host_ids)
                        home_host_slot_ids_considered = [
                            str(s.id)
                            for s in sorted_slots
                            if s.host_location_id in active_owned_participant_host_ids
                        ...
                        if true_home_host_candidate:
                            score += 1000000
                            reason_bits.append('hard-priority true home-host placement at active owned host location (+1000000)')
                        elif hosting_community_game and home_host_slot_ids_considered:
                            score -= 1000000
                            warning_bits.append('hosting-community team has a valid active owned host slot; neutral/other-community placement is not allowed unless hard constraints exhaust home-host options (-1000000)')
                        ...
                        if same_community and not hosted_by_own_community:
                            ...
                            score -= 500
                            warning_bits.append('same-community game away from active owned host location (-500)')
                            home_host_ids = active_owned_host_ids_a or ({preferred_home_host_id} if preferred_home_host_id else set())
                        ...
                        all_candidates.append({
                            ...
                            'compatible_home_slots_at_scheduling_time': (
                                sum(
                                    ...
                                    and s.host_location_id in active_owned_host_ids_a
                                    and _first_compatible_open_slot_by_field_order(s.host_location_id, s.slot_date, s.start_time) is not None
                                )
                                if same_community else 0
                            ),
                            'hosting_community_game': hosting_community_game,
                            'true_home_host_candidate': true_home_host_candidate,
                            'home_host_slots_considered': home_host_slot_ids_considered,
                            'home_host_exception_reason': None if true_home_host_candidate or not hosting_community_game or home_host_slot_ids_considered else 'No compatible active owned host slot survived hard constraints for this placement pass.',
                        })

            home_host_pairs = {c['pair'] for c in all_candidates if c.get('hosting_community_game') and c.get('true_home_host_candidate')}
            if home_host_pairs:
                all_candidates = [
                    c for c in all_candidates
                    if not (c.get('hosting_community_game') and c['pair'] in home_host_pairs and not c.get('true_home_host_candidate'))
                ]
        ...
        if valid_candidates:
            ...
            if global_home_host_pairs:
                valid_candidates = [
                    ...
                    if not (c.get('hosting_community_game') and c['pair'] in global_home_host_pairs and not c.get('true_home_host_candidate'))
                ]
        ...
        plans.append({
            ...
            'true_home_host_priority': bool(best.get('true_home_host_candidate')),
            'home_host_slots_considered': list(best.get('home_host_slots_considered') or []),
            'home_host_exception_reason': best.get('home_host_exception_reason'),
        })
        if (
            selected_host_ids
...
def auto_schedule_entire_season(payload: dict, current_user: User = Depends(require_roles(ROLE_LEAGUE_ADMIN)), db: Session = Depends(get_db)):
    ...
    def _week_field_size_counts(week_id: uuid.UUID) -> dict[str, int]:
        counts = {FIELD_SIZE_SMALL: 0, FIELD_SIZE_MEDIUM: 0, FIELD_SIZE_LARGE: 0}
        rows = db.query(Game.id, GameSlot.field_type, Field.layout_type, FieldInstance.field_type).select_from(Game).join(Game.status).outerjoin(
            GameSlot, GameSlot.assigned_game_id == Game.id
        ).outerjoin(Field, Field.id == Game.field_id).outerjoin(FieldInstance, FieldInstance.id == Game.field_instance_id).filter(
            Game.season_id == season_id,
            Game.week_id == week_id,
            GameStatus.code != 'UNSCHEDULED',
            GameStatus.is_active.is_(True),
        ).all()
        for _game_id, slot_field_type, field_layout_type, field_instance_type in rows:
            size = _normalize_field_size(slot_field_type) or _normalize_field_size(field_instance_type) or _normalize_field_size(field_layout_type) or str(slot_field_type or field_instance_type or field_layout_type or '').upper()
            if size in counts:
                counts[size] += 1
        return counts
    ...
    def _build_root_cause_categories(diagnostics: dict[str, object]) -> list[str]:
        ...
        unresolved_games = max(0, expected_games - existing_scheduled)
        if no_active_teams:
        ...
        if unresolved_games > 0 and int(diagnostics.get('host_availability_total') or 0) == 0:
        ...
        if unresolved_games > 0 and missing_slot_sizes:
        ...
        if unresolved_games > 0 and compatible_slot_count == 0 and not missing_slot_sizes:
            categories.add('no_compatible_slots')
        if unresolved_games > 0 and scheduled_count == 0 and skipped_count > 0:
            categories.add('placement_blocked_by_constraints')
        if unresolved_games > 0 and placement_attempts > 0 and committed_count == 0:
        ...
        if not categories and unresolved_games == 0 and validation_failures == 0 and database_failures == 0:
            return []
        if not categories:
    ...
    def _build_auto_schedule_diagnostics() -> dict[str, object]:
        ...
        for division in active_divisions:
            ...
            active_teams_by_division.append({
                ...
                'active_team_count': team_count,
                'no_bye_doubleheaders_enabled': _division_no_bye_doubleheaders_enabled(division),
                'expected_games_per_week': _division_required_games(team_count, division),
                'active_teams_expected_to_play_per_week': _division_active_teams_expected_to_play(team_count, division),
        ...
        for week in regular_weeks:
            ...
            for division in active_divisions:
                ...
                expected_games_by_division_week.append({
                    ...
                    'active_team_count': active_team_count_by_division.get(division.id, 0),
                    'active_teams_expected_to_play': _division_active_teams_expected_to_play(active_team_count_by_division.get(division.id, 0), division),
                    'team_appearances_expected': required_games * 2,
                    'no_bye_doubleheaders_enabled': _division_no_bye_doubleheaders_enabled(division),
        ...
        diagnostics = {
            ...
            'committed_games_count': existing_scheduled_games_total,
            'failed_validation_count': 0 if (sum(expected_games_by_week.values()) > 0 and existing_scheduled_games_total >= sum(expected_games_by_week.values())) else max(failed_validation_count, sum(failed_validation_reasons.values())),
            'failed_validation_reasons': {} if (sum(expected_games_by_week.values()) > 0 and existing_scheduled_games_total >= sum(expected_games_by_week.values())) else dict(failed_validation_reasons),
            'database_commit_failed_count': database_commit_failed_count,
            'dry_run': dry_run,
            'scheduled_games_count': existing_scheduled_games_total,
    ...
    for group, name in division_order:
        ...
        for week in _regular_season_weeks():
            ...
            division_week_diagnostic = {
                ...
                'host_activation_summary': placement_preview_diagnostic.get('host_activation_summary') if placement_preview_diagnostic else missing_preview_diagnostic.get('host_activation_summary'),
                'placement_attempt_details': placement_attempt_details,
    ...
    for week in _regular_season_weeks():
        ...
        expected_games_for_week = 0
        active_teams_expected_to_play = 0
        team_appearances_expected = 0
        expected_games_by_field_size = _empty_field_size_counts()
        for division in _active_divisions_for_auto_schedule():
            active_count = _active_team_count(division.id)
            division_expected_games = _division_required_games(active_count, division)
            expected_games_for_week += division_expected_games
            active_teams_expected_to_play += _division_active_teams_expected_to_play(active_count, division)
            team_appearances_expected += division_expected_games * 2
            required_size = _required_field_type_for_division(division)
            if required_size in expected_games_by_field_size:
                expected_games_by_field_size[required_size] += division_expected_games
        missing_total = max(0, expected_games_for_week - scheduled_games_total)
        missing_by_size, extra_by_size = _diagnostic_field_size_diff(expected_games_by_field_size, field_size_counts)
        team_appearance_counts: dict[uuid.UUID, int] = {}
        for game in scheduled_games:
            for team_id in (game.home_team_id, game.away_team_id):
                if team_id:
                    team_appearance_counts[team_id] = team_appearance_counts.get(team_id, 0) + 1
        doubleheader_teams_count = sum(1 for count in team_appearance_counts.values() if count > 1)
        bye_teams_count = max(0, active_teams_expected_to_play - unique_teams_playing)
        week_diagnostic = weekly_diagnostics_by_week_id.setdefault(week.id, {
            'week': week.week_number,
            'week_start_date': str(week_game_date) if week_game_date else None,
            'expected_games_total': 0,
        ...
        week_diagnostic['expected_games_total'] = expected_games_for_week
        week_diagnostic['scheduled_games_total'] = scheduled_games_total
        week_diagnostic['missing_games_total'] = missing_total
        week_diagnostic['active_teams_expected_to_play'] = active_teams_expected_to_play
        week_diagnostic['unique_teams_playing'] = unique_teams_playing
        week_diagnostic['team_appearances_expected'] = team_appearances_expected
        week_diagnostic['team_appearances_actual'] = team_appearances_actual
        week_diagnostic['doubleheader_teams_count'] = doubleheader_teams_count
        week_diagnostic['bye_teams_count'] = bye_teams_count
        week_diagnostic['field_size_counts'] = field_size_counts
        week_diagnostic['expected_games_by_field_size'] = expected_games_by_field_size
        week_diagnostic['actual_games_by_field_size'] = field_size_counts
        week_diagnostic['missing_games_by_field_size'] = missing_by_size
        week_diagnostic['extra_games_by_field_size'] = extra_by_size
        missing_divisions = [
            division_row
        ...
        validation_row = {
            'week': week.week_number,
            'week_start_date': str(week_game_date) if week_game_date else None,
            'expected_games': expected_games_for_week,
            'scheduled_games': scheduled_games_total,
            'active_teams_expected_to_play': active_teams_expected_to_play,
            'unique_teams_playing': unique_teams_playing,
            'team_appearances_expected': team_appearances_expected,
            'team_appearances_actual': team_appearances_actual,
            'doubleheader_teams_count': doubleheader_teams_count,
            'bye_teams_count': bye_teams_count,
            'expected_games_by_field_size': expected_games_by_field_size,
            'actual_games_by_field_size': field_size_counts,
            'missing_games_by_field_size': missing_by_size,
            'extra_games_by_field_size': extra_by_size,
        ...
        if scheduled_games_total != expected_games_for_week:
            validation_errors.append(
                f'Week {week.week_number}: scheduled_games={scheduled_games_total}, expected_games={expected_games_for_week}, missing_games={missing_total}.'
            )
        if unique_teams_playing != active_teams_expected_to_play:
            validation_errors.append(
                f'Week {week.week_number}: unique_teams_playing={unique_teams_playing}, active_teams_expected_to_play={active_teams_expected_to_play}.'
            )
        if team_appearances_actual != team_appearances_expected:
            validation_errors.append(
                f'Week {week.week_number}: team_appearances_actual={team_appearances_actual}, team_appearances_expected={team_appearances_expected}.'
            )
        if any(missing_by_size.values()):
            validation_errors.append(
                f'Week {week.week_number}: field-size demand not met; missing_games_by_field_size={missing_by_size}.'
            )
        if missing_total > 0 and not any(row.get('week') == week.week_number for row in required_games_missing):
            required_games_missing.append({
                'division': 'WEEK_TOTAL',
                'week': week.week_number,
                'required_games': expected_games_for_week,
    ...
    for division_row in division_week_diagnostics:
        ...
        if isinstance(summary, dict) and summary.get('scheduling_date'):
            sanitized_summary = {key: value for key, value in summary.items() if key != 'selected_host_uuid_set'}
            host_activation_summaries_by_date[str(summary.get('scheduling_date'))] = sanitized_summary
    if host_activation_summaries_by_date:
        auto_schedule_diagnostics['host_activation_summary_by_date'] = list(host_activation_summaries_by_date.values())
        auto_schedule_diagnostics['Host Activation Summary'] = list(host_activation_summaries_by_date.values())
    root_cause_categories = list(auto_schedule_diagnostics.get('root_cause_categories') or [])
    informational_notes: list[str] = []
    if auto_schedule_diagnostics.get('all_required_games_already_scheduled'):
        informational_notes.append('All required game groups were already scheduled before this pass.')
    if int(auto_schedule_diagnostics.get('expected_games_total') or 0) > 0 and total_games_created == 0 and not dry_run and not auto_schedule_diagnostics.get('all_required_games_already_scheduled'):
        warning_message = 'Auto-schedule completed but no games were scheduled.'
        if 'no_generated_slots' in root_cause_categories:
    ...
    if auto_schedule_diagnostics.get('all_required_games_already_scheduled'):
        validation_errors = [error for error in validation_errors if 'no compatible generated slots' not in str(error).lower() and 'no generated slots' not in str(error).lower()]
    ...
    diagnostic_complete = (
        ...
        and failed_validation_total == 0
        and division_week_counts_complete
        and not validation_errors
    )

    final_status = 'complete' if diagnostic_complete else 'incomplete'
    final_message = 'Auto-schedule completed successfully. All required games are scheduled.' if diagnostic_complete else 'Auto-schedule completed with missing required games; review validation_failures and diagnostics.'
    if dry_run:
    ...
    elif total_games_created == 0 and not diagnostic_complete:
    ...
    return {
        ...
        'dry_run': dry_run,
        'preview_games_count': preview_games_count,
        'committed_games_count': committed_games_count,
        'scheduled_games_count': scheduled_games_count,
...
def _effective_score_status(game: Game, score: GameScore | None = None) -> str:
    active_score = score if score is not None else getattr(game, 'score', None)
...
def _ensure_scheduled_game(db: Session, game_id: uuid.UUID) -> Game:
    ...
    if str(game.status.code or '').strip().lower() == 'unscheduled':
        raise HTTPException(400, 'Scores can only be entered for scheduled games.')
    return game
...
def _score_submission_dict(submission: ScoreSubmission) -> dict:
    return {
        ...
        'submission_source_community_id': str(submission.submission_source_community_id) if submission.submission_source_community_id else None,
        'submission_source_community_name': getattr(getattr(submission, 'submission_source_community', None), 'name', None),
        'community_admin_notes': submission.community_admin_notes,
...
def _build_turf_stadium_utilization_diagnostics(db: Session, season_id: uuid.UUID | None = None) -> list[dict[str, object]]:
    ...
    for wave, host, availability in rows:
        ...
        wave_row = {
            ...
            'expected_capacity_by_size': {size: int(available_counts.get(size) or 0) for size in FIELD_SIZE_ORDER if int(available_counts.get(size) or 0) > 0},
            'assigned_by_size': {size: int(placed_counts.get(size) or 0) for size in FIELD_SIZE_ORDER if int(placed_counts.get(size) or 0) > 0},
            'unused_by_size': {size: int(unused_counts.get(size) or 0) for size in FIELD_SIZE_ORDER if int(unused_counts.get(size) or 0) > 0},
            'needed_field_sizes': [size for size in FIELD_SIZE_ORDER if int(unused_counts.get(size) or 0) > 0],
