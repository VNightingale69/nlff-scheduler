...
class HostLocation(Base, TimestampMixin):
    ...
    surface_type: Mapped[str] = mapped_column(String(40), nullable=False, default='GRASS_FIELD')
    max_small_fields: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    max_medium_fields: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    ...
    overflow_activation_allowed: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ...
    location_image_url: Mapped[str | None] = mapped_column(String(500))
    location_image_filename: Mapped[str | None] = mapped_column(String(255))
    location_image_storage_key: Mapped[str | None] = mapped_column(String(500))
    location_image_updated_at: Mapped[DateTime | None] = mapped_column(DateTime(timezone=True))
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    organization = relationship('Organization')
...
class HostLocationConfiguration(Base, TimestampMixin):
    ...
    surface_type: Mapped[str] = mapped_column(String(40), nullable=False, default='TURF_STADIUM')
    space_used_yards: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    remaining_yards: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    large_field_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
...
class FieldConfigurationOption(Base, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(String(120), nullable=False)
    configuration_name: Mapped[str | None] = mapped_column(String(120))
    surface_type: Mapped[str] = mapped_column(String(40), nullable=False, default='GRASS_FIELD')
    space_used_yards: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    remaining_yards: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    large_field_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
...
class HostingAvailability(Base, TimestampMixin):
    ...
    auto_select_turf_layout: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    lock_selected_layout: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    allow_turf_layout_changes: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    admin_override_incompatible_field_size: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class Game(Base, TimestampMixin):
    ...
    missing_field_assignment: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    needs_schedule_review: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
...
class GameScore(Base, TimestampMixin):
    ...
    submitted_by_community_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('organizations.id'), nullable=True)
    ...
    score_conflict: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    confirmed_by_opponent: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    flagged: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    flag_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    flagged_by_user_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id'), nullable=True)
    flagged_by_community_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('organizations.id'), nullable=True)
    flagged_at: Mapped[DateTime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_updated_by_user_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id'), nullable=True)
    ...
    submitted_by_community = relationship('Organization', foreign_keys=[submitted_by_community_id])
    approved_by = relationship('User', foreign_keys=[approved_by_user_id])
    published_by = relationship('User', foreign_keys=[published_by_user_id])
    unpublished_by = relationship('User', foreign_keys=[unpublished_by_user_id])
    flagged_by = relationship('User', foreign_keys=[flagged_by_user_id])
    flagged_by_community = relationship('Organization', foreign_keys=[flagged_by_community_id])
    ...
    __table_args__ = (
        ...
        Index('ix_game_scores_published', 'is_published'),
        Index('ix_game_scores_conflict', 'score_conflict'),
    )
...
class ScoreHistory(Base):
    ...
    previous_is_published: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    new_is_published: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    actor_user_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('users.id'), nullable=True)
    actor_role: Mapped[str | None] = mapped_column(String(100), nullable=True)
    actor_community_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey('organizations.id'), nullable=True)
    reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    game = relationship('Game', back_populates='score_history')
    actor = relationship('User', foreign_keys=[actor_user_id])
    actor_community = relationship('Organization', foreign_keys=[actor_community_id])

    __table_args__ = (
        Index('ix_score_history_game_id', 'game_id'),
        Index('ix_score_history_action', 'action'),
        Index('ix_score_history_created_at', 'created_at'),
    )
